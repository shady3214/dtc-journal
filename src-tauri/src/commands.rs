...
#[tauri::command]
pub fn get_calendar_month(app: AppHandle, year: i32, month: u32) -> Result<Vec<CalendarDayStat>, String> {
    let conn = db::connection(&app)?;
    let month_prefix = format!("{year:04}-{month:02}");
    let mut stmt = conn.prepare("SELECT substr(opened_at,1,10), COUNT(*), COALESCE(SUM(pnl),0) FROM trades WHERE opened_at LIKE ?1 || '%' GROUP BY substr(opened_at,1,10)").map_err(|e| e.to_string())?;
    let rows = stmt.query_map(params![month_prefix], |row| Ok(CalendarDayStat { date: row.get(0)?, trades: row.get(1)?, pnl: row.get(2)? })).map_err(|e| e.to_string())?;
    rows.collect::<Result<Vec<_>, _>>().map_err(|e| e.to_string())
}

#[tauri::command]
pub fn get_analytics(app: AppHandle) -> Result<AnalyticsSummary, String> {
    let conn = db::connection(&app)?;
    let mut stmt = conn.prepare("SELECT pnl, opened_at FROM trades WHERE status='Closed' ORDER BY opened_at ASC").map_err(|e| e.to_string())?;
    let rows = stmt.query_map([], |r| Ok((r.get::<_, f64>(0)?, r.get::<_, String>(1)?))).map_err(|e| e.to_string())?;
...
pub fn analyze_trade_image(app: AppHandle, trade_id: String, _image_path: String) -> Result<AiAnalysis, String> {
    let conn = db::connection(&app)?;
    let trade = list_trades(app)?.into_iter().find(|t| t.id == trade_id).ok_or("Trade not found".to_string())?;
